...
const WordToPDFConverter = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      if (!selectedFile.name.toLowerCase().match(/\.(doc|docx)$/)) {
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (droppedFile) {
      if (!droppedFile.name.toLowerCase().match(/\.(doc|docx)$/)) {
        toast.error("Please upload a Word document (.doc or .docx)");
        return;
      }
      setFile(droppedFile);
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (userEmail) {
        ...
        const convertedName = file.name.replace(/\.(doc|docx)$/i, ".pdf");
          ...
          <TabsContent value="convert" className="space-y-6">
            {!file ? (
              ...
              >
                ...
                <h3 className="text-lg font-medium">Upload a Word document</h3>
